...
contract SimpleSwap {
    ...
    address public immutable permit;
    ...
    constructor(ISwapRouter02 _swapRouter, address _permit) {
    ...
    function swapWETHForWEWE(uint amountIn) external returns (uint256 amountOut) {
        ...
        TransferHelper.safeApprove(WETH9, permit, amountIn);
    ...
    function swapWEWEtoWETH(uint amountIn) external returns (uint256 amountOut) {
        ...
        TransferHelper.safeApprove(WEWE, permit, amountIn);
        console.log('safeApprove');
        // Create the params that will be used to execute the swap
        IV3SwapRouter.ExactInputSingleParams memory params =
            IV3SwapRouter.ExactInputSingleParams({
